            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted">Manage invoices and billing for all clients</p>
                    </div>
            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="text-muted">View and manage your pet care invoices</p>
                    </div>
                </div>
            </div>
        </div>
...
function renderInvoicesList(invoices) {
    ...
    return invoices
        ...
        .map(invoice => `
            <div class="invoice-card mb-4 invoice-item" data-status="${invoice.status}" data-date="${invoice.date}">
                <div class="invoice-header">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <div class="invoice-number mb-2">${invoice.number}</div>
                            <h5 class="fw-bold mb-1">${invoice.clientName}</h5>
                            <p class="text-muted mb-0">Pet: ${invoice.petName}</p>
                        </div>
                        <div class="text-end">
                            <div class="h4 fw-bold text-success mb-1">$${invoice.total.toFixed(2)}</div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <p class="text-muted mb-1 small">Subtotal: $${invoice.subtotal.toFixed(2)}</p>
                        <p class="text-muted mb-1 small">Tax: $${invoice.tax.toFixed(2)}</p>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <button class="btn btn-outline-primary btn-sm" onclick="viewInvoiceDetails('${invoice.id}')">
                            <i class="fas fa-eye me-1"></i>View Details
                        </button>
                        <button class="btn btn-outline-success btn-sm" onclick="downloadInvoicePDF('${invoice.id}')">
                            <i class="fas fa-download me-1"></i>Download PDF
                        </button>
                    </div>
                    ${AppState.userType === 'admin' && invoice.status === 'pending' ? `
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" class="form-control" id="invoice-date" value="${new Date().toISOString().split('T')[0]}" required>
                                </div>
                                <div class="col-md-6 mb-3">
...
function loadClientPets() {
    ...
    if (clientId) {
        const clientPets = JSON.parse(localStorage.getItem(`bbc_clinic_pets_${clientId}`) || '[]');
...
function handleCreateInvoice(event) {
    ...
    serviceRows.forEach(row => {
        const description = row.querySelector('[name="service-description"]').value.trim();
        const quantity = parseInt(row.querySelector('[name="service-quantity"]').value) || 0;
    ...
    const invoiceNumber = `BBC-${new Date().getFullYear()}-${String(Date.now()).slice(-6)}`;
