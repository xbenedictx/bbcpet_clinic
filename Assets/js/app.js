...
function loadUserData() {
    ...
    const savedPets = localStorage.getItem(`bbc_clinic_pets_${AppState.currentUser.id}`);
    if (savedPets) {
        AppState.pets = JSON.parse(savedPets);
    }
    
    const savedAppointments = localStorage.getItem(`bbc_clinic_appointments_${AppState.currentUser.id}`);
    if (savedAppointments) {
        AppState.appointments = JSON.parse(savedAppointments);
    }
    
    const savedHealthRecords = localStorage.getItem(`bbc_clinic_health_records_${AppState.currentUser.id}`);
    if (savedHealthRecords) {
        AppState.healthRecords = JSON.parse(savedHealthRecords);
    }
    
    const savedInvoices = localStorage.getItem(`bbc_clinic_invoices_${AppState.currentUser.id}`);
    if (savedInvoices) {
...
function saveUserData() {
    ...
    localStorage.setItem(`bbc_clinic_pets_${AppState.currentUser.id}`, JSON.stringify(AppState.pets));
    localStorage.setItem(`bbc_clinic_appointments_${AppState.currentUser.id}`, JSON.stringify(AppState.appointments));
    localStorage.setItem(`bbc_clinic_health_records_${AppState.currentUser.id}`, JSON.stringify(AppState.healthRecords));
    localStorage.setItem(`bbc_clinic_invoices_${AppState.currentUser.id}`, JSON.stringify(AppState.invoices));
}
...
function initializeSampleData() {
    ...
    if (!userExists) {
        ...
        const demoPets = [
            {
                ...
                ownerId: 'user_001',
                medicalNotes: 'Friendly and energetic. Up to date on vaccinations.',
        ...
        localStorage.setItem('bbc_clinic_pets_user_001', JSON.stringify(demoPets));
        ...
        localStorage.setItem('bbc_clinic_appointments_user_001', JSON.stringify(demoAppointments));
        ...
        const demoHealthRecords = [
            {
                ...
                prescription: 'Heartgard Plus - 1 tablet monthly for heartworm prevention',
                weight: 30,
                temperature: 101.5,
                heartRate: 120,
        ...
        localStorage.setItem('bbc_clinic_health_records_user_001', JSON.stringify(demoHealthRecords));
        ...
        const demoInvoices = [
            {
                ...
                date: '2024-01-10',
                services: [
                    { description: 'Annual Wellness Exam', quantity: 1, price: 75.00 },
                    { description: 'DHPP Vaccination', quantity: 1, price: 25.00 },
                    { description: 'Rabies Vaccination', quantity: 1, price: 20.00 },
                    { description: 'Heartgard Plus (6 months)', quantity: 1, price: 45.00 }
                ],
                subtotal: 165.00,
                tax: 13.20,
                total: 178.20,
        ...
        localStorage.setItem('bbc_clinic_invoices_user_001', JSON.stringify(demoInvoices));
    }
}
